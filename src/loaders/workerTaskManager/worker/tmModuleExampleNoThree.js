...
class WTMModuleExampleNoThree {
	...
	static execute ( context, id, config ) {
		...
		const sender = new GeometryTransport( 'execComplete', config.id ).setGeometry( geometry, 1 ).package( false );
...
self.addEventListener( 'message', message => WorkerTaskManagerDefaultRouting.comRouting( self, message, WTMModuleExampleNoThree, 'init', 'execute' ), false );

export { WTMModuleExampleNoThree }
